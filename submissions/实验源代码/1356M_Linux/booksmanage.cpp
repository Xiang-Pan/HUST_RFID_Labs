...
{
    QString LabelName[] = {"卡号：", "书名：", "作者：", "出版社：", "总数（本）", "剩余（本）","可借阅时间（天）"};//标签文本
...
//    EditLayout->addWidget()
    ...
    Edit[Borrow_Days]->setValidator(new QRegExpValidator(regExp, this));//可借天数为三位数
    ...
    stat_Button = new QPushButton();
    stat_Button->setText("统计书籍借阅信息");
    ButtonLayout->addWidget(stat_Button);
...
void BooksManage::SetSlot()//设置槽函数
{
    ...
    connect(stat_Button,SIGNAL(clicked()),this,SLOT(stat_books()));//stat_books()
...
void BooksManage::add_books()//添加按钮槽函数
{
    ...
    //向数据库中添加书籍
    bool ret = sql->InsertBooks(Edit[ID_Books]->text(),Edit[Name_Books]->text(),Edit[Author_Books]->text(),Edit[PublishingHouse_Books]->text(),Edit[Count_Books]->text().toInt(),residue,Edit[Borrow_Days]->text().toInt());
    if(!ret)
    {
...
{    
    ...
    //修改书籍信息
    bool ret = sql->UpdataBooks(Edit[ID_Books]->text(),Edit[Name_Books]->text(),Edit[Author_Books]->text(),Edit[PublishingHouse_Books]->text(),Edit[Count_Books]->text().toInt(), Edit[Residue_Books]->text().toInt(),Edit[Borrow_Days]->text().toInt());
    if(!ret)
    {
...
{
    Table->setHorizontalHeaderLabels(QStringList()<<"卡号"<<"书名"<<"作者"<<"出版社"<<"总计（本）"<<"剩余（本）"<<"可借阅时间（天）");
    if(!query.next())
    {
...
void BooksManage::stat_books()
{
    stat_dialog* statdialog=new stat_dialog(this);
    statdialog->sql=sql;
    statdialog->exec();
    //window.
