...
Borrow_Return::Borrow_Return(QWidget *parent) : QWidget(parent)
{
    ...
    sql->InsertRecord("123", "789",borrow_time,NULL);
    sql->InsertRecord("123", "999",borrow_time,NULL);
    sql->InsertRecord("123", "456",borrow_time,NULL);
...
void Borrow_Return::SetInfo(QString cardID)
{
    ...
    if(query.next())//如果是书
    {
        ...
        else
        {
            ...
            QString sql_cmd="SELECT * FROM record_15693 WHERE cardID = "+Edit_User[CardId_User_Borrow]->text()+" AND booksID = "+query.value(0).toString()+" AND return_time ~= """;
            QSqlQuery myquery;
            myquery.exec(sql_cmd);
            if(!query.next())//query is empty
            {
                QMessageBox::warning(NULL, "warning", "还书错误！", QMessageBox::Yes, QMessageBox::Yes);
                return;
            }
