...
//    5D096B2C000104E0
//    692A6B2C000104E0
    //SetCardRecord("5D096B2C000104E0","892A6B2C000104E0");
    while(nRunFlag)
    {
        ...
        if(serialport->bytesAvailable() >= 4) //只有串口中数据有4个字节时才开始读取
        {
            ...
            SetCardRecord("5D096B2C000104E0","892A6B2C000104E0");
//            SetCardRecord("5D096B2C000104E0","12345678910");
 ...
 * QString 转十六进制数组
 */
void UHF_Thread::str2hex(const QString &str, quint8 *buf, quint8 len)
{
...
{
    //QMessageBox::warning(NULL, "warning", "写卡", QMessageBox::Yes, QMessageBox::Yes);
    ...
    quint8 * buf1;
    buf1=new quint8[8];
    str2hex(userID,buf1,8);
    memcpy(vdata1+1,buf1,8);    //write ID
    vdata1[9]=0x02;  //add
    quint8 * buf2;
    buf2=new quint8[4];
    str2hex(bookID,buf2,4);
    memcpy(vdata1+10,buf2,4);   //record ID
//    QString a;
    ...
    quint8 * buf3;
    buf3=new quint8[8];
    str2hex(userID,buf3,8);
    memcpy(vdata2+1,buf3,8);    //write ID
    vdata2[9]=0x03;  //address
    quint8 * buf4;
    buf4=new quint8[8];
    str2hex(bookID.remove(0,4),buf4,8);
    memcpy(vdata2+10,buf4,4);   //record ID
...
bool UHF_Thread::GetCardRecord(QString userID,QString bookID)
{
    ...
    uint16 cmd = RC632_CMD_ISO15693_READ_SM;  //read单个块
    uint8* vdata1;
    vdata1 = new uint8[14];
    vdata1[0]=0x02;  //cut
