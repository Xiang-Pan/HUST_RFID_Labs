...
void Widget::setSlot()
{
    ...
    connect(borrow_return, SIGNAL(set_record(QString, QString)), this, SLOT(Set_User_Record(QString, QString)));
    connect(uhf, SIGNAL(cycle()), this, SLOT(Get_User_Info()), Qt::BlockingQueuedConnection);//刷卡响应连接到槽函数Get_User_Info()
    //connect(uhf, SIGNAL(cycle()), this, SLOT(Get_Info()), Qt::BlockingQueuedConnection);//刷卡响应连接到槽函数Get_User_Info()
...
//        qDebug() << "fcs:"<<data.fcs;
        bool ok = true;
        //if(Dll.RC632_UartCalcFCS(((uint8*)(Info.data()+4)),BUILD_UINT8(Info.at(3),Info.at(2))-1) == data.fcs.toUInt(&ok,2))//判断检验和
        if(1)
        {
//            qDebug() << " 计算：" << Dll.RC632_UartCalcFCS(((uint8*)(Info.data()+4)),BUILD_UINT8(Info.at(3),Info.at(2))-1);
//            qDebug() << " raw : " << data.fcs.toUInt(&ok,2);
    ...
    if(Dll.RC632_AnalysisFrame((uint8*)(Info.data()),RC632_CMD_ISO15693_READ_SM) != 0xff)//判断是否是读卡命令
    {
        QString record = data.vdata.replace(" ","");
        qDebug()<<record<<endl;
    }
...
void Widget::Set_User_Record(QString UserID,QString BookID)
{
    //QMessageBox::warning(NULL, "warning", "调用写卡！", QMessageBox::Yes, QMessageBox::Yes);
    qDebug()<<"!!!!!"<<UserID<<BookID;
    uhf->SetCardRecord(UserID,BookID);
}


void Widget::Get_User_Record(QString UserID,QString BookID)
{
    uhf->GetCardRecord(UserID,BookID);
}
