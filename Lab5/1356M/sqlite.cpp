...
bool Sqlite::Connect()
{
    ...
    query.exec("create table books_15693 (booksID vchar, name vchar, author vchar, publishing_house vchar, count int, residue int, availableTime int, primary key (booksID))");
    query.exec("create table record_15693 (cardID vchar, booksID vchar, brrowDate vdate, FOREIGN KEY (cardID ) REFERENCES user(cardID), FOREIGN KEY (booksID ) REFERENCES user(booksID))");
    return true;
}
//打印SQL语句
...
//向books表中添加
bool Sqlite::InsertBooks(QString booksID, QString name, QString author, QString publishing_house, int count, int residue, int available)
{
    return Insert("books_15693(booksID, name, author, publishing_house, count, residue, availableTime)", "'"+booksID+"', '"+name+"', '"+author+"', '"+publishing_house+"', "+QString::number(count)+", "+QString::number(residue)+", "+QString::number(available));
}
//向record表中添加
bool Sqlite::InsertRecord(QString cardID, QString booksID)
{
    ...
    return Insert("record_15693(cardID, booksID, brrowDate)", "'"+cardID+"', '"+booksID+"','"+StrDate+"'");
}
