...
{
    QString LabelName[] = {"卡号：", "书名：", "作者：", "出版社：", "总数（本）", "剩余（本）", "可借时长（天）"};//标签文本
...
void BooksManage::add_books()//添加按钮槽函数
{
    ...
    /*文本框为空时显示错误提示*/
    QString LabelName[] = {"卡号：", "书名：", "作者：", "出版社：", "总数（本）", "可借时长（天）"};
    ...
    //向数据库中添加书籍
    bool ret = sql->InsertBooks(Edit[ID_Books]->text(),Edit[Name_Books]->text(),Edit[Author_Books]->text(),Edit[PublishingHouse_Books]->text(),Edit[Count_Books]->text().toInt(),residue,Edit[Available_Books]->text().toInt());
    if(!ret)
    {
...
{
    //设置表头
    Table->setHorizontalHeaderLabels(QStringList()<<"卡号"<<"书名"<<"作者"<<"出版社"<<"总计（本）"<<"剩余（本）"<<"可借时长（天）");
    if(!query.next())
    {
