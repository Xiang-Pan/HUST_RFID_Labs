...
UserManage::UserManage(QWidget *parent) : QWidget(parent)
{
    ...
    QString ButtonName[] = {"注册","挂失/激活", "删除", "修改", "搜索"}; //按钮文本
    ...
    Edit[Tele_User]->setValidator(new QRegExpValidator(regExp,this));
...
void UserManage::SetSlot()
{
    ...
    connect(Button[Loss_User],SIGNAL(clicked()),this,SLOT(lossOrReget_user()));//挂号按钮连接槽函数
...
{
    QString LabelName[] = {"卡号：", "姓名：", "性别：", "年龄：","手机号：","状态："};
     for(int i = 0; i < Edit_Count_USER; i++)
     {
    ...
    int ret = sql->InsertUser(Edit[ID_User]->text(),Edit[Name_User]->text(),Edit[Gender_User]->text(),Edit[Age_User]->text().toInt(),Edit[Tele_User]->text(),1);
    if(!ret)
    {
...
//挂失或激活用户槽函数
void UserManage::lossOrReget_user()
{
    //为了安全，需提供卡号、姓名、手机号
    QSqlQuery query = sql->SelectUser(Edit[ID_User]->text(),Edit[Name_User]->text(),Edit[Tele_User]->text(),NULL,-1);
    if(!query.next())
    {
        QMessageBox::warning(NULL, "warning", "卡号不存在！", QMessageBox::Yes, QMessageBox::Yes);
        return;
    }
    bool willStatus = false;
    QString warn = "挂失";
    if(query.value("status").toBool() == false)
    {
        willStatus = true;
        warn = "激活";
//        QMessageBox::warning(NULL, "warning", "该卡已注销！", QMessageBox::Yes, QMessageBox::Yes);
//        return;
    }
    //改变卡的状态
    int ret = sql->UpdataUserStatus(Edit[ID_User]->text(),willStatus);
    if(!ret)
    {
        QMessageBox::warning(NULL, "warning", warn+"失败！", QMessageBox::Yes, QMessageBox::Yes);
        return;
    }
    QMessageBox::warning(NULL, "warning", warn+"成功！", QMessageBox::Yes, QMessageBox::Yes);
...
void UserManage::updata_user()
{
    ...
    int ret = sql->UpdataUser(Edit[ID_User]->text(),Edit[Name_User]->text(),Edit[Gender_User]->text(),Edit[Age_User]->text().toInt(),Edit[Tele_User]->text(),Edit[Status_User]->text().toInt());
    if(!ret)
    {
...
void UserManage::select_user()
{
    ...
    if(Edit[Age_User]->text().isEmpty())//如果年龄为空 调用SelectUser时 不传入年龄  默认年龄为-1
        ...
        query = sql->SelectUser(Edit[ID_User]->text(),Edit[Name_User]->text(),Edit[Tele_User]->text(),Edit[Gender_User]->text(),Edit[Age_User]->text().toInt());
    ShowTable(query);
}
...
{
    //设置表头
    Table->setHorizontalHeaderLabels(QStringList()<<"卡号"<<"姓名"<<"性别"<<"年龄"<<"手机号"<<"状态");
    if(!query.next())
    {
